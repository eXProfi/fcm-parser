...
def convert_fcm(file_obj):
    ...
    try:
        # gr.File with type=\"filepath\" returns a string path; support both str and Path-like.
        file_path = Path(file_obj)
        stem = file_path.stem or "converted"
        svg_path, thumbnail_path = _fcm_bytes_to_files(file_path.read_bytes(), stem)
...
with gr.Blocks(title="FCM → SVG") as demo:
    ...
    with gr.Row():
        fcm_input = gr.File(label="FCM файл", file_types=[".fcm"], type="filepath")
