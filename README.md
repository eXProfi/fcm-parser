...
You can run a simple local web app for converting FCM files to SVG using a Python 3.11.9 virtual environment (from the repository root):
...
The batch script first changes into the repository folder, activates `venv`, runs the app, and deactivates the environment when the process ends. The app opens in your default browser via `demo.launch(inbrowser=True)`. Upload an FCM file to see the SVG preview and download both the SVG and the embedded thumbnail (`.bmp`).
